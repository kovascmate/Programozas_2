...
///-----------------------------------------------------
    ...
    Vektor(InputIterator first, InputIterator last):Array<T, maxsiz>(first, last){}
    /// push_back
    /// vektor végéhez adatot ad
    /// @param val - adat
    void push_back(const T& val){this->at(Array<T, maxsiz>::size())=val;}
    /// back
    /// vektor utolsó adatának elérése
    /// @return referencia az utolsó adat
    T& back(){return this->at(Array<T,maxsiz>::size()-1);}

    /// pop_back
    /// vektor utolsó adatának eldobása
    void pop_back(){Array<T,maxsiz>::setsize(Array<T,maxsiz>::size()-1);}

    /// empty
    /// @return true, ha nincs adat
    bool empty() const {return Array<T,maxsiz>::size() == 0;}
