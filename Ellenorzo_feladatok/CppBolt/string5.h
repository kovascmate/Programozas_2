...
/// ...
    ...
    {
        this->len = 1;
        this->pData = new char[this->len+1]{ez,'\0'};

    }
    String(char const *ez)
    {
        this->len = strlen(ez);
        this->pData = new char[this->len+1];
        strcpy(this->pData,ez);
    }
    String(String const& asd)
    {
        this->len = asd.len;
        this->pData = new char[this->len+1];
        strcpy(this->pData,asd.pData);
    }
    String& operator=(String const& asd)
    {
        if(&asd == this)
            return *this;
        delete[] this->pData;
